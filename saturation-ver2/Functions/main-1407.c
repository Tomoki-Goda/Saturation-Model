...
////////////////////////  Minimalization procedure MIGRAD //////////////////////////////////
	...
	//sprintf(command , "SET PRINTOUT 2");
	//MNCOMD(fcn,command,error_flag,0);
...
//////////////////// Loop ////////////////////////////////////
	for(int i=0; i<5;i++){
...
#if SUDAKOV>=1	
		if(i!=0){		
			sprintf(command , "SET PARAMETER 5 0.4D0");
			MNCOMD(fcn,command,error_flag,0);
			sprintf(command , "FIX 5");
			MNCOMD(fcn,command,error_flag,0);
		}
		...
		N_SIMPS=(int)(N_SIMPS_R*1.0/2.0);
		SIGMA_PREC=DGAUSS_PREC*10;
		generate_psi_set();
		
		//sprintf(command,"HESSE %d", 10*(N_PAR*N_PAR)); 
		//MNCOMD(fcn,command,error_flag,0);
	
		sprintf(command , "MIGRAD %d %fD0",100*(N_PAR*N_PAR),1.0);
		MNCOMD(fcn,command,error_flag,0);
...
#if SUDAKOV>=1	
		sprintf(command , "RELEASE 5");
		MNCOMD(fcn,command,error_flag,0);
		sprintf(command , "SET LIMITS 5");
		MNCOMD(fcn,command,error_flag,0);
#endif
#if MODEL==3	
		sprintf(command , "RELEASE 2");
		MNCOMD(fcn,command,error_flag,0);
		sprintf(command , "SET LIMITS 2");
		MNCOMD(fcn,command,error_flag,0);
#endif
		sprintf(command,"HESSE %d", 10*(N_PAR*N_PAR)); 
		MNCOMD(fcn,command,error_flag,0);
		
		sprintf(command , "MINIMIZE %d %fD0",200*(N_PAR*N_PAR),1.0);
		MNCOMD(fcn,command,error_flag,0);
