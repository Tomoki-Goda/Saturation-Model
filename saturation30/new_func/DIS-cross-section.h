...
//	double r=R/(1-R);
	...
	//printf("%f\n",Q2);
                ...
                value*=1.0/r; //1 from polar coordinates, -2 from compensating psi
        #elif FLAVOUR==1
		value =SIGMA(r,mod_x(x,Q2,'l'), Q2, param) * psisq_z_int(r, Q2, 'l');
		value+=SIGMA(r,mod_x(x,Q2,'s'), Q2, param) * psisq_z_int(r, Q2, 's');
...
double sigma_DIS(double x,double q2,double y, double * par) {
        ...
        *(param+1)=par;
        ...
        double err=0;
        double ep=1.0e-6;
        simpson1dA(& sigma_r_integrand, param,0.0+ep, 30.0,200,&res,&err);

        return res;
}
/////////////////////// 2 D integration...?///////////////////////
//If 1D method doesn't work, alternatively there is always do 2 D as 1+1 D...
#elif ZINTEGRATE==0 
