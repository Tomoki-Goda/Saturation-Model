...
def main():
    ...
    #for k in ['100','650']:
    for j in ['4']:
        with open(args[0]+'/gluon-650-'+j+'.txt' ,"r") as fi:
            dpi=[]
            ri=[]
            for i in fi:
                data=i.strip().split("\t")
        ...
        with open(args[1]+'/gluon-100-'+j+'.txt',"r") as fi:
            dpi=[]
            ri=[]
            for i in fi:
                data=i.strip().split("\t")
        ...
        with open(args[1]+'/gluon-650-'+j+'.txt' ,"r") as fi:
            dpi=[]
            ri=[]
            for i in fi:
                data=i.strip().split("\t")
    ...
    ax1.grid('true')
    ax1.set_ylabel('$\\alpha_s f(x k^2)$',rotation="horizontal",loc='top')
