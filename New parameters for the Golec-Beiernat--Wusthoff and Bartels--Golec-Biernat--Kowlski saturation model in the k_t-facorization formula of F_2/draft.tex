...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\comment{To be reduced... rewritten ... }
%{
%\color{gray}
%For decades Deep Inelastic Scattering (DIS) has played major role in the studies of particle physics. The structure of a proton is one of the key elements in the particle collider experiments. In the collinear factorization, one factorizes the total cross section in to the perturbatively calculable short-distance part and a process-independent long-distance part, called parton distribution functions~(PDFs). The energy scale dependence of the PDFs are described by the Dokshitzer--Gribov--Lipatov--Alterelli--Parisi~(DGLAP) evolution equations, which resums the large logarithms. While an inclusive process can be described well by the collinear PDFs, one needs transverse momentum dependent PDFs for less inclusive processes. The PDFs also denpends on so-called Bjorken scaling variable, $x$. While the $Q$ dependence of the PFDs are perturbatively calculable, one generally needs to extract $x$ dependence from experiments. In the limit of small-$x$, however, the Balitsky--Fadin--Kuraev--Lipatov (BFKL) equation predicts that, the gluon density rises like $\sim x^{-\lambda}$.   This rise eventually violates the Froissart bound, which limit the rise of the cross section to be $\sim\log^2(s)$.  
%This  taming of the rise is called saturation. The saturation phenomena becomes relevant when the gluon density inside the proton becomes so high that recombination of them can no longer be neglected.
%There has been much effort trying to understand this effect in both perturbative evolution equations and modeling.
%On the perturbative calculation side, notably, the  Balistki--Kovchegov~(BK) equation extends the BFKL equation by including a non-linear term to encapsulate the saturation. On the modeling side, among others, Golec-Biernat and W\"usthoff (GBW) proposed a simple model, whose key essence is encoded in the $x$ dependent saturation scale, $Q_s(x)$. The saturation scale can be thought of as a line which roughly separate the saturation region and the scaling region.  It is this GBW model, which we are primarily interested in this study.
%}
\comment{Bibliography will be properly made once the draft is complete. For only now I use biblatex.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%\section{$k_t$-factorization formula}
\section{Introduction}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[t]
...
\end{equation}
%While the behaviour of $\mathcal{F}$ is described by evolution equations such as Balistky--Fadin--Kuraev-Lipatov (BFKL)~\cite{Balitsky:1978ic, Kuraev:1977fs}  and Ciafaloni--Catani--Fiorani--Marchesini (CCFM)~\cite{Ciafaloni:1987ur,Catani:1989sg,Catani:1989yc}. 
...
%Let us remind ourselves a few points about the dipole-factorization.
...
%\comment{something about large $x$ region should be mentoned. Particularly that of BGK.}
...
%, and we will not do anything in particular in this study to modify the large-$x$ region.
...
%As demonstrated below, this factorization can be transformed into the position space factorization with a new object, namely, the dipole cross section~\cite{Golec-Biernat:1998zce, Bartels:2002cj}.
%In short, the formula of Ref.~\cite{Golec-Biernat:1998zce} can essentially be obtained by replacing $x/z$ with $\tilde{x}$ of Ref.~\cite{Golec-Biernat:1998zce} and assuming fixed coupling.\\
