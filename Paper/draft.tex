...
The PDFs are not perturbatively calculable but they are process independent~\cite{collins_2011}. That is to say, that one needs to measure them in experiments, but the PDFs obtained one experiment can be used as input in another. A type of experiments called deep inelastic scattering~(DIS), has been one of experiments which largely contributed to the study of the structure of the proton~\cite{roberts_1990,collins_2011}. 
...
In Fig.~\ref{fig:GBW-BGK}, the plots of the dipole cross section, the dipole gluon density and the saturation scale are shown for the GBW and BGK models are shown. The dipole cross section and the gluon density are both normalized by $\sigma_0$ in order to see the effects  of other parameters better,
...
\end{equation}
with the hard function $H_{\gamma^*g\rightarrow q\overline{q}}$ describing interactions of an off-shell photon with an on-shell gluon to a $q\overline{q}$ pair.
...
It was shown in Refs.~\cite{Mueller:2012uf,Mueller:2013wwa,Xiao:2017yya} that consistent resummation such logarithms are possible owing to the separation of corresponding regions.  
...
ITMD is an generalization of TMD where the momentum imbalance in TMD is restricted to be small~\cite{Kotko:2015ura,vanHameren:2016ftb}. That is to say ITMD resums $(Q_s/k_T)^n$ and $(k_T/P_T)^n$~\cite{Kotko:2015ura,vanHameren:2016ftb}, thus extends the region of applicability up to $k_T\sim P_T$.
...
We are grateful to Krzysztof Golec-Biernat for numerous useful discussions and for careful reading of the manuscript, and Piotr Kotko and Andreas van Hameren \comment{appropriate phrase }. The project is partially supported by 
