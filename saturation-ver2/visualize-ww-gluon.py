...
def main():
    ...
    #for k in ['100','650']:
    for j in ['4']:
        with open(args[0]+'/ww-gluon-650-'+j+'.txt' ,"r") as fi:
            dpi=[]
            ri=[]
            for i in fi:
                data=i.strip().split("\t")
        ...
        with open(args[1]+'/ww-gluon-100-'+j+'.txt',"r") as fi:
            dpi=[]
            ri=[]
            for i in fi:
                data=i.strip().split("\t")
        ...
        with open(args[1]+'/ww-gluon-650-'+j+'.txt' ,"r") as fi:
            dpi=[]
            ri=[]
            for i in fi:
                data=i.strip().split("\t")
    ...
    ax1.legend([leg[0][0],leg[1][0],leg[2][0]],['Without Sudakov','With Sudakov $Q^2=100GeV^2$','With Sudakov $Q^2=650GeV^2$'])
    #ax1.set(title="",  ylabel="$\\alpha_s f(x k^2)$",    xlabel="$k^2$",  xscale= 'log' ,   yscale='linear' )
    ax1.set( xscale= 'log' ,   yscale='linear' )
    ax1.grid('true')
    ax1.set_ylabel('$\\Phi(x, k^2)$',rotation="vertical",loc='top')
    ax1.set_xlabel("$k^2$",rotation="horizontal",loc='right')
