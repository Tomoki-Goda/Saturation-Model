...
class Clenshaw_Curtis:public Clenshaw_Curtis_FIX{
	...
	private:
		//
		double  integrate_cc(double(*func)(double*,void*),void* args,double min,double max,int sect, double eps, double epsabs)const{
			...
			while(true){
				...
				if(( fabs(valfull-valhalf)<eps*(fabs(valfull)) ) || (  fabs(valfull-valhalf)<epsabs )||(counter>=MAX_RECURSION)){//Need improvement
					if((!(( fabs(valfull-valhalf)<eps*(fabs(valfull)) ) || (  fabs(valfull-valhalf)<epsabs ))) && counter>=MAX_RECURSION&&warn_flag<3){
							...
							for(int i=0;i<(N+1);i++){
								dummy=-cos(i*PI/N)*(max-min)+(max+min);
								dummy/=2;
...
public:
	...
	std::string name="";
 	...
 	double	integrate(double(*func)(double*,void*),void* args,double min,double max, int sect,double eps,double epsabs)const{
		return( integrate_cc(func, args, min, max,sect,  eps, epsabs));
	}
	double  integrate(double(*func)(double*,void*),void* args,double min,double max,int sect, double eps)const{
		return (integrate_cc(func, args, min, max, sect,  eps, eps));
	}
	double	integrate(double(*func)(double*,void*),void* args,double min,double max, double eps,double epsabs)const{
		return( integrate_cc(func, args, min, max,1,  eps, epsabs));
	}
	double  integrate(double(*func)(double*,void*),void* args,double min,double max, double eps)const{
		return (integrate_cc(func, args, min, max, 1,  eps, eps));
	}
